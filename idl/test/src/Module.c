...
corto_void _test_Module_setup(test_Module this) {
...
corto_void _test_Module_tc_module(test_Module this) {
/* $begin(test/Module/tc_module) */
    ...
    test_assert(corto_package(m)->url == NULL);
    corto_release(m);
...
corto_void _test_Module_tc_nestedModule(test_Module this) {
/* $begin(test/Module/tc_nestedModule) */
    ...
    corto_interface i = corto_interface(s);
    test_assert(i->members.length == 1);
    test_assert(i->members.buffer[0]->type == corto_type(corto_string_o));
    test_assert(i->members.buffer[0]->modifiers == CORTO_GLOBAL);
    test_assert(!strcmp(corto_idof(i->members.buffer[0]), "m"));
    corto_release(s);
